...
| GitLab CE | `gitlab/gitlab-ce:19.4.0-ce.0` | current minor at build time; CE = pure open source ([ADR-0004](adr/0004-git-server.md)) | source code, CI/CD | 80 (HTTP), 22 (SSH, published as 2222) |
| XWiki | `xwiki:17.10.13-postgres-tomcat` | **LTS** branch: longer fix support; the branch extensions are typically tested against. Runs as root inside the container with all capabilities dropped ([ADR-0013](adr/0013-xwiki-container.md)) | documentation | 8080 |
| OpenProject | `openproject/openproject:16.6.10-slim` + `memcached:1.6.45-alpine` | current 16.6 patch; `-slim` = application only, one container per role, non-root — the all-in-one image is documented as not for production ([ADR-0012](adr/0012-openproject-container.md)) | project management | 8080 |
...
Host engine as installed by [`scripts/bootstrap.sh`](../scripts/bootstrap.sh) on 2026-09-18 (Debian 13,
Docker apt repository): `docker-ce` 29.8.1, `containerd.io` 2.3.5,
`docker-compose-plugin` 5.5.1, `docker-buildx-plugin` 0.37.1. Only
`docker-ce`/`docker-ce-cli` are pinned to `5:29.*` ([`docs/security.md`](security.md));
...
| GitLab | OpenProject | HTTPS via Caddy | webhook on push, merge request, comment and pipeline events; GitLab allows outbound requests to `pm.lab.test` only ([ADR-0014](adr/0014-gitlab-openproject-webhook.md), [`docs/integration.md`](integration.md) §2) |
| XWiki | OpenProject | HTTPS via Caddy | OpenProject macro (work package lists) — *extension step, not built* ([`docs/integration.md`](integration.md) §3) |
...
work. [`scripts/bootstrap.sh`](../scripts/bootstrap.sh) is the tested implementation of this list for
...
| `userns-remap` | user `dockremap`, subordinate range `100000:65536` in `/etc/subuid` and `/etc/subgid`, set **before the first image pull** | container root is unprivileged on the host ([ADR-0002](adr/0002-os-and-docker.md)); the fixed range makes every bind-mount owner reproducible across rebuilds ([P-008](problems.md#p-008--dockremap-received-the-same-subordinate-range-as-the-admin-user), [P-009](problems.md#p-009--first-image-pulled-before-userns-remap-was-active)) |
...
([`docs/security.md`](security.md), "Host").
...
- **Trust model:** defence in depth with one identity source — not a Zero
  ...
  assessment and the target architecture are in [ADR-0006](adr/0006-identity.md), "Trust model
  assessment".
...
| Protocols default to `h1 h2 h3`; restricted here to `h1 h2` | [Global options, "servers"](https://caddyserver.com/docs/caddyfile/options) | HTTP/3 would need UDP/443 published and allowed; deliberately off (see [P-006](problems.md#p-006--http3-still-listed-on-the-port-80-redirect-listener-accepted)) |
